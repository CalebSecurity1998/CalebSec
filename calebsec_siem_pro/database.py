...
def init_db():
    ...
    ensure_column(cur, "alerts", "tactic", "TEXT DEFAULT ''")
    ensure_column(cur, "alerts", "risk_score", "INTEGER DEFAULT 50")
    ...
    return [
        {
            ...
            "first_seen": row[9],
            "last_seen": row[10],
            "tactic": row[11] if len(row) > 11 else "",
            "risk_score": row[12] if len(row) > 12 else 50
        }
        for row in rows
    ]
